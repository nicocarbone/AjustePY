...
def pre_calcs():
    ...
    if n_ref <= 1:
        A = 3.084635 - 6.531194 * n_ref + 8.357854 * n_ref**2 - 5.082751 * n_ref**3 + 1.171382 *  n_ref**4
    first_nonzero_data = np.nonzero(data)[0][0] #Calculate position of first nonzero value in data
    last_nonzero_data = np.nonzero(data)[0][-1] #Calculate position of last nonzero value in data
...
def funcion_fiteo_refl(t, ups, ua, t0, back):
    ...
    array_conv = np.convolve(instru, array_teo, 'full') #Convolve with response function
    array_conv.resize(data.size) #Truncate resultiing array to the size of the experimental data
